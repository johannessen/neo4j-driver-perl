...
C<net_module>, that module will be used instead of the included module.

The included module will change in future. As of S<version 0.21>,
the default module uses L<LWP> through L<REST::Client>, but the
L<REST::Client> dependency is going to be dropped. L<LWP> might
also be swapped for another module eventually.
...
It must expect UTF-8 and must offer a method C<decode()> that
should produce C<$JSON::PP::true> and C<$JSON::PP::false> for
booleans.
