...
The intention is that all deprecated features will be removed with
the next major update to L<Neo4j::Driver>, likely in December 2024.
...
See L<"Call run() in list context" for Transaction|/"Call-run()-in-list-context1">
below for details.
...
=head2 Concurrent transactions in default HTTP sessions

I<Deprecated in version 0.39 and to be removed in 1.00.
Originally added in 0.01 as an experimental feature.>
...
The driver allows concurrent transactions on HTTP, but only when the
C<concurrent_tx> config option is enabled. This config option was
added in S<version 0.30>, along with a warning that concurrent
transactions I<without> setting C<concurrent_tx> might become fatal.
In S<version 0.39>, the default value for C<concurrent_tx> changed
to C<0> and the warning was updated to say that it I<will> become
fatal.

Making concurrent transactions without C<< concurrent_tx => 1 >>
fatal will force users to be explicit about their intentions.
This should improve interoperability between HTTP and Bolt.
...
=head1 Neo4j::Driver::Type::Temporal

I<Deprecated in version 0.42 and to be removed in 1.00.
Originally added in 0.13 as an experimental feature.>

 if ( $value->isa('Neo4j::Driver::Type::Temporal') )
   { ... }

When implementing Neo4j temporal types, C<Temporal> was split into
C<DateTime> and C<Duration>. The documentation of C<Temporal> had
always warned that temporal values were not yet implemented, and
that the module name may change eventually.

Use L<Neo4j::Types::DateTime> and L<Neo4j::Types::Duration> instead.
