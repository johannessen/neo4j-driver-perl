...
=item * Consider adding a config option like C<tls_options> to be
used as input to L<IO::Socket::SSL> by the default C<net_module>.
While this avoids the need to create a new module in simple cases,
it does introduce a potential security risk.
