...
(However, this may be difficult to do, see L<Carp/"BUGS">.
Also, p5p has plans to add real error objects to a future version of Perl.)
...
L<perlmodstyle/"Error handling and messages"> doesn't help us much. The only
other relevant option (aside from error objects and C<croak()>) on that list
is a global var, which would not be thread-safe.

Perhaps a reasonable solution would be like an C<error> config option that
accepts an error handler sub. That sub should then be expected to never return
(i.e. it should C<croak()> or C<die()>; not dying should be a usage error
that causes the driver to croak with a special message in order to force the
user to really consider error handling for themselves, and keep options for
later expansion).
...
<List::Util/"none"> (in C<config()> and C<_parse_options()>;
...
C<execute_write> and C<execute_read>
(see L<https://github.com/neo4j/neo4j-java-driver/pull/1196>).
