...
=item * L<Neo4j::Driver/"Database selection">: Make stable. While running
multiple databases is mostly an enterprise feature, even the community edition
comes with the C<system> database in addition to the main/default database,
thus switching databases must be possible somehow.
