...
=item * A uri string only containing protocol should yield the default host and
port. For example, C<< Neo4j::Driver->new('bolt:') >> should be enough to get
a driver configured for C<bolt://localhost:7687>.
