...
=item * Sessions might offer a C<close> method that automatically rolls back
and terminates all currently open transactions. However, with Bolt in
particular, there can be only one open (explicit) transaction at a time, so
this really doesn't appear to be very useful. Prolly best to get rid of it.
...
=item * Instead of simulating a result stream for HTTP, StatementResult should
simply start out detached for HTTP connections. This should improve performance
and more importantly simplify both the implementation and the documentation
(which is currently imprecise with its use of "consume").
